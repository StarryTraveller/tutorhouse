...
// Check if the form is submitted for updating the asses
if (isset($_POST['update'])) {
    $id = $_POST['assess_id'];
    $newTitle = $_POST['assess_title'];
    $newDescription = $_POST['assess_description'];
    $newSubject = $_POST['assess_subject'];
    $newLevel = $_POST['assess_level'];
    
    // Check if a new file is uploaded
    if ($_FILES['assess_attachment']['name']) {
        $newFilePath = "../files/" . $_FILES['assess_attachment']['name'];

        // Upload the new file to the server
        if (move_uploaded_file($_FILES['assess_attachment']['tmp_name'], $newFilePath)) {
            // File upload successful, update the file_path in the database
            $sql = "UPDATE assessments SET title = ?, description = ?, subject = ?, level = ?, file_path = ? WHERE id = ?";
        ...
        $sql = "UPDATE assessments SET title = ?, description = ?, subject = ?, level = ?, WHERE id = ?";
        $stmt = $con->prepare($sql);
    ...
    if ($stmt->execute()) {
        ...
        echo "Error updating assessments.";
    }
}
// Check if the asses ID is provided in the URL
if (isset($_GET['id'])) {
    $id = $_GET['id'];

    // Retrieve the asses details from the database based on the ID
    $sql = "SELECT id, title, description, subject, level, file_path FROM assessments WHERE id = ?";
    $stmt = $con->prepare($sql);
    ...
    if ($result->num_rows === 1) {
        ...
        $assess_attachment = $row['file_path'];
        // Add other fields as needed

    } else {
        // asses not found with the provided ID, display an error message or handle it accordingly
   ...
   <div class="sidebar">
    ...
    <div class="content">
      <div class="main-content">
        ...
        <form action="editassess.php" method="POST" enctype="multipart/form-data">
          <!-- Your assess edit form fields here (title, description, subject, attachment) -->
          <!-- Example: -->
          <input type="hidden" name="assess_id" value="<?php echo $assessId; ?>">
          <input type="text" name="assess_title" placeholder="assess Title" value="<?php echo $assessTitle; ?>"
            required>
          <textarea name="assess_description" placeholder="assess Description" required><?php echo $assessDescription; ?></textarea>
          <select type="form-select" name="assess_subject" aria-label="Default select example" value="<?php echo $assessSubject; ?>" required>
            ...
            <select type="form-select" name="assess_level" aria-label="Default select example" value="<?php echo $assessLevel; ?>" required>
          ...
          <input type="file" name="assess_attachment" required>
          <button type="submit" name="update">Update assess</button>
        </form>
      </div>
  </div>
      </div>
