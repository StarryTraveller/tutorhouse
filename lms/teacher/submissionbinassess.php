...
// Get the assess ID from the URL parameter
if (isset($_GET['assess_id'])) {
    $assessId = $_GET['assess_id'];

    // Fetch the assess details from the 'assessments' table based on the assess ID
    ...
    // Fetch the submissions for the specific assess from the 'actsubmit' table
    $submissionSql = "SELECT * FROM assesssubmit WHERE assessid = '$assessId'";
    ...
    // Redirect to the previous page if the assess ID is not provided
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['submission_id']) && isset($_POST['remark'])) {
        ...
        $updateSql = "UPDATE assesssubmit SET remarks = '$remark' WHERE id = '$submissionId'";
...
<head>
  ...
  <title>Teacher | Assessment Bin</title>
</head>
  ...
  <div class="container-wrapper">
    ...
    <div class="content">
      <div class="main-content">
                <h1><?php echo $assess['title']; ?> Submissions</h1>
                <h5>Subject: <?php echo $assess['subject']; ?></h5>
                <h5>Level: <?php echo $assess['level']; ?></h5>
...
<?php else: ?>
    <p>No submissions found for this assess.</p>
<?php endif; ?>
            </div>
